...
The API logs to stdout and, as structured JSON, to `logs/app.log`. The `app-log` channel defined at `.rngo/channels/app-log.yml` has no effects writing to it, so it's a pure signal source: it tails that file for the duration of the run, recording each log line as a signal with no particular effect.
