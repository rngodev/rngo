...
Don't write a generic plan from the requirements alone — go look at the code. Find the files, modules, and existing patterns the change will touch (for this repo, start from the architecture notes in CLAUDE.md: `Spec` / `Dialect` in `crates/rngo`, `Simulation` / `Effect` / `Schema` in `crates/effect`, `Proxy` / channel dispatch in `crates/proxy`, `Audit` / `Signal` in `crates/audit`, the run log in `crates/log`, and the CLI run loop in `crates/cli`). A plan that could apply to any codebase isn't grounded enough yet.
