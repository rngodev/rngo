...
Don't write a generic plan from the requirements alone — go look at the code. Find the files, modules, and existing patterns the change will touch (for this repo, start from the architecture notes in CLAUDE.md: `Spec` / `Dialect::parse_merge_effect` / `MergeEffect` / `SourceEffect` / `Event` / `EventLog` in `crates/rngo`, the CLI run loop and channel dispatch in `crates/cli`). A plan that could apply to any codebase isn't grounded enough yet.
